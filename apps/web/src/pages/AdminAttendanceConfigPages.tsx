...
export function AdminAttendanceSchedulesPage() {
  ...
  const [items, setItems] = useState<Array<{ id: string; name: string; startTime: string; endTime: string; lateGraceMinutes: number; earlyLeaveToleranceMinutes: number; active: boolean; workLocationId: string | null; workLocationName: string | null }>>([]);
  const [form, setForm] = useState({ name: "", startTime: "08:00", endTime: "16:00", lateGraceMinutes: "10", earlyLeaveToleranceMinutes: "0", workLocationId: "" });
  const [editing, setEditing] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: "", startTime: "08:00", endTime: "16:00", lateGraceMinutes: "0", earlyLeaveToleranceMinutes: "0", workLocationId: "", active: true });
  ...
  return (
    <AttendanceWorkforceShell section="schedules" title="Jadwal & Shift" description="Template jam kerja, grace keterlambatan, toleransi pulang, lokasi, dan shift overnight.">
      ...
      <section className="rounded-2xl border border-border/70 bg-white p-5 shadow-[var(--shadow-soft)]">
        ...
        <p className="mt-2 text-xs text-muted-foreground">Jika jam selesai lebih kecil atau sama dengan jam mulai, sistem menganggap shift melewati tengah malam.</p>
        <button className="mt-3 inline-flex h-11 items-center gap-2 rounded-xl bg-brand-primary px-4 text-sm font-bold text-white disabled:opacity-50" disabled={busy || !form.name} onClick={() => void run(async () => {
          await createAttendanceSchedule({ name: form.name, startTime: form.startTime, endTime: form.endTime, lateGraceMinutes: Number(form.lateGraceMinutes), earlyLeaveToleranceMinutes: Number(form.earlyLeaveToleranceMinutes), workLocationId: form.workLocationId || null });
          setForm({ name: "", startTime: "08:00", endTime: "16:00", lateGraceMinutes: "10", earlyLeaveToleranceMinutes: "0", workLocationId: "" });
        ...
        <div className="overflow-x-auto"><table className="min-w-[950px] w-full text-left text-sm"><thead className="bg-surface text-xs text-muted-foreground"><tr><th className="p-3">Jadwal</th><th className="p-3">Jam</th><th className="p-3">Grace</th><th className="p-3">Pulang</th><th className="p-3">Lokasi</th><th className="p-3">Status</th><th className="p-3">Aksi</th></tr></thead><tbody className="divide-y">
          {items.map((item) => editing === item.id ? (
            <tr key={item.id}>
              ...
              <td className="p-3"><input className="h-9 w-20 rounded-lg border px-2" type="number" value={editForm.lateGraceMinutes} onChange={(e) => setEditForm({ ...editForm, lateGraceMinutes: e.target.value })} /></td>
              ...
              <td className="p-3"><div className="flex gap-2"><button className="rounded-lg bg-brand-primary p-2 text-white" onClick={() => void run(async () => { await updateAttendanceSchedule(item.id, { name: editForm.name, startTime: editForm.startTime, endTime: editForm.endTime, lateGraceMinutes: Number(editForm.lateGraceMinutes), earlyLeaveToleranceMinutes: Number(editForm.earlyLeaveToleranceMinutes), workLocationId: editForm.workLocationId || null, active: editForm.active }); setEditing(null); }, "Jadwal diperbarui.")}><Save className="h-4 w-4" /></button><button className="rounded-lg border p-2" onClick={() => setEditing(null)}><X className="h-4 w-4" /></button></div></td>
            </tr>
          ) : (
            <tr key={item.id}><td className="p-3 font-semibold">{item.name}</td><td className="p-3">{item.startTime.slice(0,5)}–{item.endTime.slice(0,5)}{item.endTime <= item.startTime ? " · overnight" : ""}</td><td className="p-3">{item.lateGraceMinutes} mnt</td><td className="p-3">{item.earlyLeaveToleranceMinutes} mnt</td><td className="p-3">{item.workLocationName ?? "—"}</td><td className="p-3">{item.active ? "Aktif" : "Nonaktif"}</td><td className="p-3"><button className="inline-flex items-center gap-1 text-xs font-bold text-brand-primary-deep" onClick={() => { setEditing(item.id); setEditForm({ name: item.name, startTime: item.startTime, endTime: item.endTime, lateGraceMinutes: String(item.lateGraceMinutes), earlyLeaveToleranceMinutes: String(item.earlyLeaveToleranceMinutes), workLocationId: item.workLocationId ?? "", active: item.active }); }}><Pencil className="h-3.5 w-3.5" /> Edit</button></td></tr>
          ))}
        </tbody></table></div>
      </section>
    </AttendanceWorkforceShell>
